...
const deepLXMethod = String.raw`}async DeepLX(e=[],t=this.Source,a=this.Target,n=this.API){e=Array.isArray(e)?e:[e],t=this.#T.DeepL[t]??this.#T.DeepL[t?.split?.(/[-_]/)?.[0]]??t?.toUpperCase?.(),a=this.#T.DeepL[a]??this.#T.DeepL[a?.split?.(/[-_]/)?.[0]]??a?.toUpperCase?.();let s=String(n?.Endpoint??"").trim();if(!s)throw new Error("DeepLX Endpoint is required");const i=String(n?.Token??n?.Key??n?.Auth??"").trim(),o=/^https?:\/\/api\.deeplx\.org(?=\/|$)/i.test(s);if(o){const e=new URL(s),t=e.pathname.split("/").filter(Boolean),a="translate"===t[t.length-1]?.toLowerCase()&&t.length>1;if(!a){if(!i)throw new Error("DeepLX API key is required for api.deeplx.org");e.pathname="/"+encodeURIComponent(i)+"/translate",e.search="",e.hash="",s=e.toString()}}else /\/translate(?:[?#]|$)/i.test(s)||(s=s.replace(/\/+$/,"")+"/translate");const r={url:s,headers:{Accept:"application/json","User-Agent":"DualSubs","Content-Type":"application/json"},body:JSON.stringify({text:e.join("||"),source_lang:t||"auto",target_lang:a})};return i&&!o&&(r.headers.Authorization="Bearer "+i),await l(r).then(t=>{if(Number(t?.statusCode??t?.status)>=400)throw new Error("DeepLX HTTP "+(t.statusCode??t.status));const n=JSON.parse(t.body);if(void 0!==n?.code&&200!==Number(n.code))throw new Error("DeepLX API "+n.code+": "+(n?.message??n?.msg??"request failed"));const s=n?.data??n?.translation??n?.translations;if(Array.isArray(s))return s.map(e=>"string"==typeof e?e:e?.text??e?.translation??"");if("string"!=typeof s)throw new Error("DeepLX response does not contain translated text");const r=s.split(/\s*\|\|\s*/);if(1===e.length)return[s];if(r.length!==e.length)throw new Error("DeepLX returned "+r.length+" segments for "+e.length+" inputs");return r}).catch(e=>Promise.reject(e))}async BaiduFanyi(`;
